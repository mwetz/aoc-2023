...
fn update_dst_vec(src: &Vec<StartRange>, dst: &Vec<StartRange>, src_start: i64, dst_start: i64, rng: i64) -> (Vec<StartRange>, Vec<StartRange>) {
    ...
    for (j, k) in src.iter().zip(dst.iter()) {
        if j.start < src_start &&  src_start + rng < j.start + j.range {
            ...
            let src_before = StartRange{start: j.start, range: src_start - j.start};
            let dst_before = StartRange{start: k.start, range: src_start - j.start};
            let src_inside = StartRange{start: src_start, range: rng};
            let dst_inside = StartRange{start: src_start - src_start + dst_start, range: rng};
            let src_after = StartRange{start: j.start + (src_start - j.start) + rng, range: j.start + j.range - (src_start + rng)};
            let dst_after = StartRange{start: k.start + (src_start - j.start) + rng, range: j.start + j.range - (src_start + rng)};
            ...
            let src_inside = StartRange{start: j.start, range: j.range};
            let dst_inside = StartRange{start: j.start - src_start + dst_start, range: j.range};
        ...
        } else if j.start >= src_start && j.start < src_start + rng && j.start + j.range > src_start + rng {
            // Only start in map => generate 2 parts
            let src_inside = StartRange{start: j.start, range: (src_start + rng) - j.start};
            let dst_inside = StartRange{start: j.start - src_start + dst_start, range: (src_start + rng) - j.start};
            let src_after = StartRange{start: j.start + (src_start - j.start) + rng, range: j.start + j.range - (src_start + rng)};
            let dst_after = StartRange{start: k.start + (src_start - j.start) + rng, range: j.start + j.range - (src_start + rng)};
            src_new.extend(vec![src_inside, src_after]);
            dst_new.extend(vec![dst_inside, dst_after]);
        } else if j.start < src_start &&  src_start < j.start + j.range && j.start + j.range <= src_start + rng {
            // Only end in map => generate 2 parts
            let src_before = StartRange{start: j.start, range: src_start - j.start};
            let dst_before = StartRange{start: k.start, range: src_start - j.start};
            let src_inside = StartRange{start: src_start, range: j.start + j.range - src_start};
            let dst_inside = StartRange{start: src_start - src_start + dst_start, range: j.start + j.range - src_start};
            src_new.extend(vec![src_before, src_inside]);
            dst_new.extend(vec![dst_before, dst_inside]);
        } else {
            // Nothing in map => return original part
            let src = StartRange{start: j.start, range: j.range};
            let dst = StartRange{start: k.start, range: j.range};
...
fn run(input: String) -> i64 {
    ...
    for l in input.lines() {
        ...
        if re_values.is_match(l) {
            ...
            let (new_src, new_dst) = update_dst_vec(prodmap.get(src).unwrap(), prodmap.get(dst).unwrap(), src_start, dst_start, rng);
            prodmap.insert(src, new_src);
            prodmap.insert(dst, new_dst);
        }
    }
    let location = prodmap.get(dst).unwrap().iter().map(|x| x.start).collect_vec();
