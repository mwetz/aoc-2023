...
fn run(input: String) -> u64 {
    ...
    let time: u64 = li.next().unwrap().split(":").last().unwrap().split(" ").filter(|x| x.len() > 0).join("").parse().unwrap();
    let dist: u64 = li.next().unwrap().split(":").last().unwrap().split(" ").filter(|x| x.len() > 0).join("").parse().unwrap();
