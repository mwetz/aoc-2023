...
impl Number {
    ...
    fn has_symbol(&self, grid:&Grid) -> bool {
        ...
        neighbours.iter().map(|n|grid.is_point_symbol(n.0, n.1)).any(|n| n)
    }
}
...
fn parse_input(input: String) -> (Grid, Vec<Number>) {
    ...
    for (y, l) in input.lines().enumerate() {
        ...
        for (x, i) in l.chars().enumerate() {
            ...
            match i {
                '.' => gridpoints.push(Point{x: x as i32, y: y as i32, ele: Element::Dot}),
                '0'..='9' => gridpoints.push(Point{x: x as i32, y: y as i32, ele: Element::Digit}),
                _ => gridpoints.push(Point{x: x as i32, y: y as i32, ele: Element::Symbol}),
            }
            ...
            match i {
                ...
                _ => {
                    if number.len() > 0 {
                        numbers.push(Number{y: y as i32, xmin: x as i32 - number.len() as i32, xmax: x as i32 - 1 as i32, value: String::from_iter(number).parse().unwrap()});
                        number = Vec::new();
                    }
                }
            }
        ...
        if number.len() > 0 {
            numbers.push(Number{y: y as i32, xmin: maxline as i32 - number.len() as i32, xmax: maxline as i32 - 1 as i32, value: String::from_iter(number).parse().unwrap()});
        }
    }
...
fn run(input: String) -> u32 {
    ...
    // }
    let sum:u32 = numbers.iter().map(|n| n.value * n.has_symbol(&grid) as u32).sum();
    return sum
}
